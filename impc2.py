...
def _ensure_ids(rows):
    """Give every entry a stable unique id (needed to target the right
    card when the delete/cross button is clicked). Returns (rows, changed)."""
    changed = False
    for r in rows:
        if not r.get("id"):
            r["id"] = uuid.uuid4().hex
        ...
        if not os.path.exists(path):
            ...
            try:
                with open(path, "r", encoding="utf-8") as f:
                    rows = _deserialize(json.load(f))
            except (json.JSONDecodeError, OSError):
                rows = []

    rows, changed = _ensure_ids(rows)
    if changed and rows:
        # backfill ids for entries created before this feature existed
        save_improvements(model_key, rows)
    return rows
...
def _inject_css():
    st.markdown(
        ...
        /* Each improvement card is now a real st.container(key="card_...")
           so its delete button can actually run Python. This selector
           matches every one of those containers generically; the accent
           left-border colour is added per-card via a tiny inline <style>
           tag right before each container is drawn. */
        ...
        div[class*="st-key-card_"]:hover { box-shadow: 0 12px 26px rgba(15, 23, 42, 0.12) !important; }

        /* Small red "trash" / confirm buttons that live inside each card */
        ...
        div[class*="st-key-card_"] div.stButton>button:hover {
        ...
        .sub-symptom { font-size: 1.05rem; font-weight: 750; color: #111827; margin: 8px 0 4px 0; }
        .meta-line { font-size: 0.82rem; color: #6b7280; margin-bottom: 4px; }
        .status-pill {
            display: inline-block; padding: 3px 10px; border-radius: 999px; font-size: 0.72rem;
            font-weight: 700; color: white; margin-top: 6px;
        }
        .items-wrap { margin-top: 12px; border-top: 1px dashed #e5e7eb; padding-top: 10px; }
        .imp-item-chip {
...
def _card_accent(entry):
    return SYMPTOM_COLORS.get(entry.get("Symptom", ""), "#7c3aed")


def _render_card_body(entry, sno):
    """HTML for everything in a card EXCEPT the top row — the top row
    (symptom tag + the real delete button) is rendered separately with
    native Streamlit widgets so the cross button actually works."""
...
def _delete_improvement(model_key, entry_id):
    store = st.session_state.improvements_store[model_key]
    st.session_state.improvements_store[model_key] = [e for e in store if e.get("id") != entry_id]
    save_improvements(model_key, st.session_state.improvements_store[model_key])
    st.session_state.pending_delete = None
    st.toast("Improvement removed", icon="🗑️")
...
def show():
    ...
    if not filtered:
        ...
        for row_entries in rows:
            ...
            for col, entry in zip(row_cols, row_entries):
                card_number += 1
                sno = card_number
                entry_id = entry.get("id")
                card_key = f"card_{entry_id}"
                accent = _card_accent(entry)

                # per-card accent colour on the left border (button styling
                # is shared/global, see _inject_css)
                st.markdown(
                    f"<style>div.st-key-{card_key} {{ border-left: 6px solid {accent} !important; }}</style>",
                    unsafe_allow_html=True,
                )
                ...
                with col:
                    with st.container(border=True, key=card_key):
                        ...
                        with top_l:
                            st.markdown(
                                f"<span class='symptom-tag' style='background:{accent};'>"
                                f"{entry.get('Symptom','')}</span> "
                                f"<span style='color:#9ca3af; font-size:0.75rem; font-weight:700;'>#{sno}</span>",
                                unsafe_allow_html=True,
                            )
                        with top_r:
                            if st.session_state.pending_delete == entry_id:
                                yc, nc = st.columns(2)
                                with yc:
                                    if st.button("✔", key=f"yes_{entry_id}", help="Confirm delete"):
                                        _delete_improvement(model_key, entry_id)
                                        st.rerun()
                                with nc:
                                    if st.button("✕", key=f"no_{entry_id}", help="Cancel"):
                                        st.session_state.pending_delete = None
                                        st.rerun()
                            else:
                                if st.button("🗑️", key=f"del_{entry_id}", help="Remove this improvement"):
                                    st.session_state.pending_delete = entry_id
                                    st.rerun()

                        if st.session_state.pending_delete == entry_id:
                            st.caption("Remove this improvement? This can't be undone.")

                        st.markdown(_render_card_body(entry, sno), unsafe_allow_html=True)
